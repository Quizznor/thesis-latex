...
THealpixMap ud_grade_healpix(THealpixMap & map_in, int nside_out)
{
  unsigned int seed;
...
vector<vector<complex<double> > > anafast_alm_healpix(THealpixMap & map, int lmax)
{
...
double integrate_nc5(const vector<double> & x, const vector<double> & y)
{
  ...
  unsigned int reste = (npts-1)-nbii*4;  
  unsigned int nbii2;
  if(reste == 1 || reste == 2) nbii2 = nbii-1;
  else nbii2 = nbii;
  ii.resize(nbii2);
  ...
  for(unsigned int i=0; i<nbii2; i++)
    {
      integrale += 2.*h*(7.*(y[ii[i]-4]+y[ii[i]])+32.*(y[ii[i]-3]+y[ii[i]-1])+12.*y[ii[i]-2])/45.;
    }
  ...
  if( reste+1 == 2 )
    {
      ...
      // 4 points
      integrale += 3*h*(y[shift-4]+3*y[shift-3]+3*y[shift-2]+y[shift-1])/8.; 
      // 3 points
      integrale += h*(y[npts-3]+4*y[npts-2]+y[npts-1])/3.;
    }
  else if( reste+1 == 3 )
    {
      // decoupage 4-4
    ...
    {
      integrale += 3*h*(y[npts-4]+3*y[npts-3]+3*y[npts-2]+y[npts-1])/8.;
    }
  return integrale;
}

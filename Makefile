...
all:
	...
	#$(LATEX) $*
clean:
	- $(RM) -f $(TARGET){.bbl,.synctex.gz,.fdb_latexmk,.pdf,.tex~,.ps,.dvi,.log,.aux,.toc,.bcf,.nav,.out,.snm,.bbl,.blg,-blx.bib,.tex.bbl,.tex.blg,.run.xml,.spl,.lof,.maf,.fff,.lot,.ttt,.mtc*,.ptc,.loc,.tdo,.soc} */*.aux *.fls

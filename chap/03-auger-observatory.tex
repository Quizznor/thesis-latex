...
\todo{ref}). If a trigger condition is met, the \FPGA stores the data in memory 
for further processing. The memory address is determined by the type of raised 
trigger. In total, four \FIFO ring buffers are available for writing. Two of 
these comprise the \texttt{ShowerBuffer}, and the remaining two form the 
\texttt{MuonBuffer}. 

A calibration trigger with a low threshold and high rate (\todo{order here}) 
writes 

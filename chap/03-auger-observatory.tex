...
upon an \TTHREE request (see \cref{sssec:sd-triggers}). In total, the two shower
...
\begin{figure}[t]
  \centering
  \includegraphics[width=1.0\textwidth]{auger-observatory/filter_and_downsample.png}
  \caption{Comparison between a \FBW \UUB trace \textit{(top)} and a \FDS \UB 
  trace \textit{(bottom)}. Both detector responses are captured from the same
  simulated signal. The \UB pulse height is on average  lower .}
  \label{fig:filter-and-downsample}
\end{figure}

\todo{add peak scaling factor to image description above}

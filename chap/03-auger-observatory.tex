...
and show a lower $S(1000)$. This is corrected via the \CIC, which applies an
attenuation function, $f(\theta)$ to $S(1000)$. $f(\theta)$ is estimated in an 
empirical procedure by comparing observed distributions of $S(1000)$ for 
showers of different inclinations to the (without attenuation) expected 
constant distribution. A parameter optimization finds

\begin{equation}
\label{eq:cic}
f(\theta) = ax^3 + bx^2 + cx + 1, \mathrm{where} x = \cos^2(\theta) - \cos^2(\SI{38}{\degree}).
\end{equation}

The values of $a$, $b$, and $c$ in \cref{eq:cic} are given in 
\cref{tab:cic-parameters}.


that reverses the zenith-dependent attenuation on an event-level.

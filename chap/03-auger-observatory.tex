...
the circular buffer. This data is the raw event data that gets sent to \CDAS
...
than $\sim\SI{8}{\second}$ are needed for \CDAS to request \TTWO information
...
\begin{figure}[t]
  ...
  \cite{assisMeasurementWaterCherenkovDetector2016}, and reveals the systematic
  difference between the two distributions. Data is from 
  \cite{sarmentoVerticalVsOmnidirectional2024}. 
  \subref{fig:filter-and-downsample} Comparison between the simulated detector 
  response of a \UUB station (top), and a \UB station (bottom). Pulse heights 
  recorded from the same signal are $\sim\SI{15}{\percent}$ lower for a \FDS
  trace compared to its' \FBW counterpart 
  \cite{convengaLocalStationCalibrationDummies2023}.
}
...
\TTHREE request from \CDAS, all histograms are sent to the central server and
used to approximate the minute values of \Ivem, \Qvem, \Imip, and \Qmip \todo{ref here}.

...
\footnote{\HEAT has three copies of the same setup for its' telescopes}.
...
the pulse start and stop via a pulse-finding algorithm (c.f. \todo{xy section 
...
\cite{salinaNewCalibrationTask2014, salinaMeetingCollaborazioneAuger2025} like

\begin{equation}
\label{eq:cal-a-propagation}
C^{\mathrm{pixel},\,i}_{\mathrm{abs.}} = \frac{S_\mathrm{LCU}}{S_\mathrm{LCU}^\mathrm{ref.}}\,\frac{S_\mathrm{CalA}^\mathrm{ref.}}{S_\mathrm{CalA}}\,c_\mathrm{LCU}^\mathrm{Corr.}\,C^\mathrm{pixel, i}_\mathrm{abs., ref}.
\end{equation}

...
\begin{table}
  ...
  \toprule
	...
	\ldelim\{{5}{*}[\parbox{4mm}{\rotatebox[origin=c]{90}{Event level}}]
	 & $E_\mathrm{SD}$ & \SI{14}{\percent} & \SD-estimate of the primary 
	 energy & \\
	 & $E_\mathrm{FD}$ & \SI{14}{\percent} & \FD-estimate of the primary
	 energy & \\ 
	 & $(\theta, \phi)$ & $<\SI{2}{\degree}$ & Arrival direction of the 
	 primary particle & \cref{sssec:reconstruction-plane-fit} \\
	 ...
	 size & \\
	 \cline{2-5}
	 \ldelim\{{9}{*}[\parbox{4mm}{\rotatebox[origin=c]{90}{Station / 
	 Telescope level}}]
	 ...
	 & $Q^\mathrm{total}_\mathrm{VEM}$ & $\sim\SI{5}{\percent}$ & 
	 Integrated signal received by the \LPMTs &
	 \cref{sssec:reconstruction-trace-calibrator} \\
	 & $Q^\mathrm{total}_\mathrm{MIP}$ & $\sim\SI{5}{\percent}$ & 
	 Integrated signal received by the \SSD \PMT & 
	 \cref{sssec:reconstruction-trace-calibrator} \\
	 & & & & \\
	 & & & & \\
	 & & & & \\
...
\label{sssec:reconstruction-energy-estimator}
...
budget, which grows from $\sigma_{E_\mathrm{FD}}=\SI{14}{\percent}$\todo{revisit FD energy uncertainty} to 
...
The next module in line is responsible for preparing \FD data for 
reconstruction. First, timing corrections are applied to the \FD-\TTWO 
timestamps in order to synchronize measurement information across all 
telescopes. 

For each pixel the baseline is evaluated as the mean \ADC values of the first 
300 samples of the corresponding trace. By subtracting the baseline and 
applying the pixel calibration constants (see \cref{ssec:fd-calibration}) the 
trace is converted from units of \ADC to numbers of photons at a reference 
wavelength. This process is done in an identical manner to 
...
of virtual pixel channels. Finally, the integration interval is determined 
similar to how the \SD signal window is calculated.

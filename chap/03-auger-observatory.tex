...
detect \CRs of incredibly high energies. It infers their existence by observing
large particle cascades in the atmosphere in a hybrid fashion. The \FD is 
sensitive to the faint fluorescence light produced in an \EAS, and the \SD 
samples the shower footprint on ground. The simultaneous operation of the \FD 
and the \SD offers a unique possibility to observe \UHECRs at the tail-end of 
the \CR energy spectrum, and makes the \PAO world-leading in the area of \UHECR
research.

This chapter introduces the observatory and the collaboration operating it. A 
summary of the project history, motivation and eventual science case of the 
experiment is given in \cref{sec:project-history-and-future}. \cref{sec:fd},
and \cref{sec:sd} detail the hardware components of the \FD and the \SD 
respectively and introduce key concepts needed to follow the analysis in later
chapters. Finally, the analysis chain from \EAS detection to the estimation
of physics parameters during the event reconstruction is examined in 
\cref{sec:event-reconstruction}.
...
\LED emits 50 light pulses with a duration of $\approx\SI{60}{\micro\second}$\todo{wtf is this symbol}
at a wavelength of \SI{470\pm25}{\nano\meter} 
\cite{menshikovLEDCalibrationUnit}. The optical signal is guided to the 
diffusers at the center of the mirrors. A seventh fiber feeds the light back to
the \LCU, where a photodiode digitizes the signal $S_\mathrm{LCU}$. The 
readout of pixel-data is forced via an external trigger coincident with every 
light pulse. The overall signal recorded in the \FD time trace from the 
...
49 bins (concluding in total 69 bins, or $\sim\SI{0.575}{\micro\second}$) from
the \WCD and \SSD \HG channels are stored, with the \FPGA alternating writing 
between both 

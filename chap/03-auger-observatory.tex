...
The study of \EAS data by \Offline is governed by steering files in \XML 
format, that dictate which software modules are executed in which order. 
Consequentially, this defines which information regarding the \EAS is 
reconstructed. Usually, at the end of the reconstruction the results of the 
analysis are written to an \ADST, a compact data structure based on the 
\texttt{root} format \cite{brunROOTObjectOriented1997}. This section follows 
the procedure of a standard hybrid event reconstruction, and details how event-
and station-level variables are calculated during the analysis. An overview of
the most important reconstructed quantities is given in 
...
\begin{table}
	...
	\EAS reconstruction.}
	\begin{tabular*}{0.9\textwidth}{M{0.7cm}M{2.3cm}|M{7.05cm}|M{2.1cm}}
  \toprule
	...
	\ldelim\{{5}{*}[\parbox{4mm}{\rotatebox[origin=c]{90}{Event level}}]
	 ...
	 & $E_\mathrm{FD}$ / \SI{14}{\percent} & Primary particle energy,
	 from the \FD & \\ 
	 ...
	 Intersect of \EAS axis and Earth surface & \\ 
	 & $\Xmax$ / \SI{50}{\percent} & Atmospheric depth at the max. \EAS 
	 size & \\
	 \cline{2-4}
	\ldelim\{{9}{*}[\parbox{4mm}{\rotatebox[origin=c]{90}{Station/Telescope
	level}}]
	 & \Ivem / \SI{2}{\percent} & Calibration factor for the \WCD \LPMTs & 
	 \cref{sssec:sd-offline-calib} \\
	 & \Imip / \SI{2}{\percent} & Calibration factor for the \SSD \PMT & 
	 \cref{sssec:sd-offline-calib} \\
 & & \\
	 & & & \\
	 & & & \\
...
\subsubsection{SD PMT calibration}
\label{sssec:sd-offline-calib}

First, the signal recovered from the \SD \PMTs, meaning the \LPMTs, \sPMT, and 

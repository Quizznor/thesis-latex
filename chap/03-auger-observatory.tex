...
\subsubsection{SD Station Calibration}
\label{sssec:sd-offline-calib}

It follows a discussion of several modules that, for the sake of clarity, are 
grouped together in one section. They all have a common purpose in processing
the raw \SD detector data, and ensuring it is of good quality.
...
Next, the dynode-to-anode ratio, as well as the \TTWO timestamp of the 
surviving stations are adjusted and corrected according to \LUTs that keep 
track of station-level software/hardware changes that influence either of the 
two quantities.
...
\cref{fig:ssd-histo}). Crucially, all histograms show a local maximum that can
be identified with the most probable peak/charge from a single atmospheric 
muon.
...
The \PMT baseline is revisited in a similar fashion. 

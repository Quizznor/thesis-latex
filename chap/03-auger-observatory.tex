...
underground. The soil above the detectors provides an excellent attenuation for 
non-muonic particles in \EASs. Particularly, this implies that the signals 
measured by the \UMD consist entirely of muons, allowing the \PAO to e.g. probe 
the muon content of \EASs \todo{cite joachims thesis}.
...
strategy for the calibration of the \UMD \EAS data is thus chosen. The \UMD does
not need to be self-calibrating, as it is run purely in a secondary mode to the
\WCD, and does not generate triggers itself 
\cite{aabCalibrationUndergroundMuon2021}.

\subsubsection{\acf{RD}}

completion citation (castellinaSeasonalGreetings202424)

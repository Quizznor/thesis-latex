...
The following sections detail the different modules that are executed during 
a standard event reconstruction for the \FD, and the \SD (including only 
information from the \WCD and the \SSD). The \UMD and the \RD are, given their 
relative novelty, not part of the well established module sequences that are 
typically in use. Details on the event reconstruction using the \UMD and the 
\RD data can be found in \cite{bottiAMIGAUndergroundMuon2021} and \todo{simons proceeding}, respectively. 
...
\begin{table}
  ...
  \toprule
	...
	\ldelim\{{5}{*}[\parbox{4mm}{\rotatebox[origin=c]{90}{Event level}}]
	 ...
	 energy & \\ 
	 & $(\theta, \phi)$ & $\SI{1}{\degree}\,$-$\,\SI{2}{\degree}$ & Arrival
...
unsaturated portion of the \PMT trace, accounting for \PMT undershoot in the 
process \cite{vebericEstimationSignalSaturated2013}.
...
greisenCosmicRayShowers1960}. asdasd
...
SD-433). $\beta$ and $\gamma$ are the shape parameters of the \LDF, and
determine the compactness of the shower. Example visualizations are offered in
\cref{fig:ldf}. $S(1000)$, also called the shower size, is the value of the 
\LDF at $r=\SI{1000}{\meter}$, and independent of the shower shape (i.e.\ of
$\beta$ and $\gamma$).
